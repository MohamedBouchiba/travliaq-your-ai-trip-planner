...
const TOOL_CONFIG: Record<string, { icon: React.ElementType; label: string; color: string }> = {
  classify_intent: { icon: Brain, label: "Analyse", color: "text-purple-500" },
  plan_response: { icon: Sparkles, label: "Réflexion", color: "text-amber-500" },
  update_flight_widget: { icon: Plane, label: "Vol", color: "text-sky-500" },
  update_accommodation_widget: { icon: Hotel, label: "Hébergement", color: "text-emerald-500" },
  update_preferences: { icon: Users, label: "Préférences", color: "text-pink-500" },
  generate_quick_replies: { icon: MessageSquare, label: "Suggestions", color: "text-indigo-500" },
  request_destination_suggestions: { icon: MapPin, label: "Destinations", color: "text-rose-500" },
  trigger_flight_search: { icon: Search, label: "Recherche", color: "text-teal-500" },
};

/**
 * Default config for unknown tools
 */
const DEFAULT_TOOL_CONFIG = { icon: Brain, label: "Traitement", color: "text-muted-foreground" };

/**
 * Get tool config with fallback
 */
function getToolConfig(toolName: string) {
  return TOOL_CONFIG[toolName] || { ...DEFAULT_TOOL_CONFIG, label: toolName };
}
...
export const ToolStatusIndicator = memo(function ToolStatusIndicator({
  ...
  const visibleTools = tools.filter(t => 
    t.status === "running" || 
    t.status === "pending" ||
    (t.status === "success" && t.duration !== undefined)
  );
